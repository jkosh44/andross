...
cluster (via [tikv/raft-rs](https://github.com/tikv/raft-rs)), so writes are consistently replicated and remain
available through node failures. Storage on each node is backed by [fjall](https://github.com/fjall-rs/fjall), a
log-structured, embeddable key-value storage engine written in Rust. This project is for fun and learning and not meant
for production use.
